...
function animateAlongPoints(points, endRect, duration) {
    ...
    if (endRect.width && endRect.height) {
        endZoom = Math.min((figma.viewport.bounds.width * figma.viewport.zoom - padding * 2) / endRect.width, (figma.viewport.bounds.height * figma.viewport.zoom - padding * 2) / endRect.height) * zoomLevel;
    }
    if (!endZoom)
        endZoom = startZoom;
...
function animateToRect(rect, duration) {
    ...
    if (rect.width && rect.height) {
        endZoom = Math.min((figma.viewport.bounds.width * figma.viewport.zoom - padding * 2) / rect.width, (figma.viewport.bounds.height * figma.viewport.zoom - padding * 2) / rect.height) * zoomLevel;
    }
    if (!endZoom)
        endZoom = startZoom;
